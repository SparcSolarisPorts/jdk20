...
// JavaThread::_cont_fastpath is unbiased while V9 SP is biased.
// The greatest address identifies the oldest known Java frame.
void MacroAssembler::push_cont_fastpath(Register java_thread) {
  if (!Continuations::enabled()) return;
  Label done;
  add(SP, STACK_BIAS, G1_scratch);
  ld_ptr(java_thread, JavaThread::cont_fastpath_offset(), G3_scratch);
  cmp_and_brx_short(G1_scratch, G3_scratch, Assembler::lessEqualUnsigned,
                    Assembler::pt, done);
  st_ptr(G1_scratch, java_thread, JavaThread::cont_fastpath_offset());
  bind(done);
}
...
void MacroAssembler::pop_cont_fastpath(Register java_thread) {
  ...
  add(SP, STACK_BIAS, G1_scratch);
  ld_ptr(java_thread, JavaThread::cont_fastpath_offset(), G3_scratch);
  cmp_and_brx_short(G1_scratch, G3_scratch, Assembler::lessUnsigned,
                    Assembler::pt, done);
  st_ptr(G0, java_thread, JavaThread::cont_fastpath_offset());
  bind(done);
}
...
void MacroAssembler::enter() {
  ShouldNotCallThis(); // unused legacy tracing hook
}
...
void MacroAssembler::debug(char* msg, RegistersForDebugging* regs) {
  if ( ShowMessageBoxOnError ) {
    ...
    BREAKPOINT;
      ThreadStateTransition::transition_from_vm(JavaThread::current(), saved_state);
  }
  else {
