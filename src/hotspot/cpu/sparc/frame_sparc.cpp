...
frame::frame(intptr_t* sp, intptr_t* younger_sp, bool younger_frame_is_interpreted) :
  ...
  _unextended_sp = sp + _sp_adjustment_by_callee;
...
intptr_t* frame::interpreter_frame_sender_sp() const {
  ...
  if (is_heap_frame()) {
    return fp() + sp()[I5_savedSP->sp_offset_in_saved_window()];
  }
  return (intptr_t*)(sp()[I5_savedSP->sp_offset_in_saved_window()] + STACK_BIAS);
}
...
UpcallStub::FrameData* UpcallStub::frame_data_for_frame(const frame& fr) const {
  assert(fr.is_upcall_stub_frame(), "wrong frame");
  // FrameData is addressed from the unextended (architectural) SP.  Convert
  // to an ordinary address before applying the byte offset.
  return reinterpret_cast<UpcallStub::FrameData*>(
      reinterpret_cast<address>(fr.unextended_sp()) + in_bytes(_frame_data_offset));
}
...
frame frame::sender_for_upcall_stub_frame(RegisterMap* map) const {
  ...
  UpcallStub* blob = _cb->as_upcall_stub();
  JavaFrameAnchor* jfa = blob->jfa_for_frame(*this);
  assert(!upcall_stub_frame_is_first(), "first upcall frame has no Java sender");
