...
#define ADLER32_NUM_TEMPS 16
  ...
  address generate_cont_thaw(Continuation::thaw_kind kind) {
    ...
    const bool return_barrier_exception = Continuation::is_thaw_return_barrier_exception(kind);
    address start = __ pc();
...
#ifdef ASSERT
    {
      ...
      __ cmp_and_brx_short(G1, G3_scratch, Assembler::equal, Assembler::pt, ok);
      __ stop("continuation thaw: incorrect SP");
      __ bind(ok);
    }
#endif

    // A return barrier may carry either an integer/oop result in O0 or an FP
    // result in F0.  Preserve both; restoring both is harmless and avoids
    // having type-specific return barriers.
    if (return_barrier) {
      __ sub(SP, 2 * wordSize, SP);
      __ st_ptr(O0, SP, STACK_BIAS + wordSize);
      __ stf(FloatRegisterImpl::D, F0, SP, STACK_BIAS);
    }

    __ set(return_barrier ? 1 : 0, L0);
    __ call_VM_leaf(L7_thread_cache,
                    CAST_FROM_FN_PTR(address, Continuation::prepare_thaw),
                    G2_thread, L0);
    __ mov(O0, L2); // bytes required for thawed frames; zero means overflow

    if (return_barrier) {
      __ ldf(FloatRegisterImpl::D, SP, STACK_BIAS, F0);
      __ ld_ptr(SP, STACK_BIAS + wordSize, O0);
      __ add(SP, 2 * wordSize, SP);
    }
    ...
    // Continuation runtime APIs use unbiased stack pointers.  Allocate below
    // the ContinuationEntry and preserve the 16-byte SPARC ABI alignment.
    __ add(SP, STACK_BIAS, L3);
    __ sub(L3, L2, L3);
    __ and3(L3, -16, L3);
    __ sub(L3, STACK_BIAS, SP);
    ...
    __ set((int)kind, L0);
    __ call_VM_leaf(L7_thread_cache,
                    Continuation::thaw_entry(),
                    G2_thread, L0);
    __ mov(O0, L1); // unbiased SP of the yielding frame
 ...
 public:
  StubGenerator(CodeBuffer* code, int phase) : StubCodeGenerator(code) {
    assert(phase >= 0 && phase <= 2, "invalid stub generation phase");
    _stub_count = phase == 0 ? 0x100 : (phase == 1 ? 0x180 : 0x200);
    if (phase == 0) {
      generate_initial();
    } else if (phase == 1) {
      generate_phase1();
...
// save_frame they are visible as %i0/%i1/%i2.
void fill_continuation_entry(MacroAssembler* masm) {
#ifdef ASSERT
  __ set(ContinuationEntry::cookie_value(), G1);
  __ st(G1, SP, STACK_BIAS + in_bytes(ContinuationEntry::cookie_offset()));
#endif
  __ st_ptr(I0, SP, STACK_BIAS + in_bytes(ContinuationEntry::cont_offset()));
  __ st(I2, SP, STACK_BIAS + in_bytes(ContinuationEntry::flags_offset()));
  __ st_ptr(G0, SP, STACK_BIAS + in_bytes(ContinuationEntry::chunk_offset()));
  __ st(G0, SP, STACK_BIAS + in_bytes(ContinuationEntry::argsize_offset()));
  __ st(G0, SP, STACK_BIAS + in_bytes(ContinuationEntry::pin_count_offset()));
...
void continuation_enter_cleanup(MacroAssembler* masm) {
  ...
  __ add(SP, STACK_BIAS, G3_scratch);
  __ cmp_and_brx_short(G1, G3_scratch, Assembler::equal, Assembler::pt, ok);
  __ stop("continuation cleanup: incorrect SP");
  __ bind(ok);
#endif
