...
void LIRGenerator::do_CompareOp(CompareOp* x) {
  ...
  if (x->x()->type()->is_float_kind()) {
    ...
    ShouldNotReachHere(); // CompareOp is defined for floating-point and long values
  }
}
...
LIR_Opr LIRGenerator::atomic_cmpxchg(BasicType type, LIR_Opr addr, LIRItem& cmp_value, LIRItem& new_value) {
  ...
  if (is_reference_type(type)) {
    ...
    ShouldNotCallThis(); // narrow primitive compare-and-set is unsupported on SPARC C1
  }
...
LIR_Opr LIRGenerator::atomic_add(BasicType type, LIR_Opr addr, LIRItem& value) {
  ShouldNotCallThis(); // SPARC C1 reports atomic get-and-add as unsupported
...
void LIRGenerator::do_vectorizedMismatch(Intrinsic* x) {
  (void)x;
  ShouldNotCallThis(); // filtered by Compiler::is_intrinsic_supported
}
