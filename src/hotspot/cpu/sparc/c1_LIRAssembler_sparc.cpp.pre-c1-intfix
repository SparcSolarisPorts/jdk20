...
void LIR_Assembler::clinit_barrier(ciMethod* method) {
  (void)method;
  ShouldNotCallThis(); // supports_fast_class_init_checks() is false on SPARC
}
...
void LIR_Assembler::monitorexit(LIR_Opr obj_opr, LIR_Opr lock_opr, Register hdr, int monitor_no) {
  ...
  // unlock object
  MonitorAccessStub* slow_case = new MonitorExitStub(lock_opr, !UseHeavyMonitors, monitor_no);
...
void LIR_Assembler::mem2reg(LIR_Opr src_opr, LIR_Opr dest, BasicType type,
  ...
  if (disp_reg == noreg) {
    offset = load(src, disp_value, to_reg, type, wide, false);
  } else {
...
void LIR_Assembler::reg2mem(LIR_Opr from_reg, LIR_Opr dest, BasicType type,
  ...
  assert(disp_reg != noreg || Assembler::is_simm13(disp_value), "should have set this up");
  if (disp_reg == noreg) {
    offset = store(from_reg, src, disp_value, type, wide, false);
  } else {
...
void LIR_Assembler::cmove(LIR_Condition condition, LIR_Opr opr1, LIR_Opr opr2, LIR_Opr result, BasicType type,
  ...
  assert(cmp_opr1 == LIR_OprFact::illegalOpr && cmp_opr2 == LIR_OprFact::illegalOpr, "unnecessary cmp oprs on sparc");
  Assembler::Condition acond;
  switch (condition) {
    case lir_cond_equal:        acond = Assembler::equal;        break;
    case lir_cond_notEqual:     acond = Assembler::notEqual;     break;
...
void LIR_Assembler::push(LIR_Opr opr) {
  (void)opr;
  ShouldNotCallThis(); // LIR stack push/pop operations are unused on SPARC
}
...
void LIR_Assembler::volatile_move_op(LIR_Opr src, LIR_Opr dest, BasicType type, CodeEmitInfo* info) {
  ...
  if (type == T_LONG) {
    ...
    if (src->is_register() && dest->is_address()) {
      ...
      ShouldNotReachHere(); // C1 emits atomic long moves only for register/address pairs
    }
    if (info != NULL) {
...
void LIR_Assembler::on_spin_wait() {
  // SPARC has no universally available pause instruction.  The VM reports
  // supports_on_spin_wait() == false, so C1 does not emit this operation.
}
