...
void os::print_tos_pc(outputStream *st, const void *context) {
  ...
  // The PC may itself be corrupt. Keep instruction decoding last, as the
  // other platform implementations do, so the captured stack is retained.
  print_instructions(st, os::Posix::ucontext_get_pc(uc), sizeof(char));
  st->cr();
}
